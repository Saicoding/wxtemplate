...
function uploads(mdl = '', callback,url){
  var datas = [];
  wx.chooseImage({
    count: 1, // 默认9
    ...
    success: function (res) {
      ...
      wx.uploadFile({
        url: url + '/api/upload/upload', //仅为示例，非真实的接口地址
...
function fileupload(mdl = '', callback) {
      var url = '';
      if (module.exports.globalData.apiurl) {
       //  console.log(module.exports.globalData.apiur);
        uploads(mdl, callback, module.exports.globalData.apiur);
      }else{
        wx.getExtConfig({
          success: function (res) {
            module.exports.globalData = res.extConfig;
            console.log(res.extConfig.apiurl);
            uploads(mdl, callback, res.extConfig.apiurl);
          }
        })
      } 
}
