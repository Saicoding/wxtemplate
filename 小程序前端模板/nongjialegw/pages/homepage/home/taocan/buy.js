    ...
    if (!myreg.test(wxb.that.data.mobile)) {
        ...
        if(!wxb.that.data.name || !wxb.that.data.idcard){
          wx.showToast({
            ...
            wxb.Post('/api/nongjialegw.order/create',{
                  id:wxb.that.data.id,
              ...
              wx.reLaunch({
                url: '/pages/homepage/me/order/detail2?order_id='+data.id,
                  })
            });
        }
    }
      ...
      if (!wxb.checkAuthLogin()) {
        wxb.login();
          ...
          if (bgend) {
            ...
            wx.redirectTo({
              url: '/pages/public/date?type=buy&id=' + wxb.that.data.id,
            })
          }
          wx.showLoading({
            title: '正在加载中',
          ...
          wxb.Post('/api/nongjialegw.order/checkOrder', {
            openid: wxb.getOpenId(),
            id: wxb.that.data.id,
            day: wxb.that.data.day,
          }, function (data) {
